 ...
 * @param maxLeaves  hard ceiling on leaves broken in one swing (valid range 0-4096; 0 breaks
 *                   none). Separate from {@code maxBlocks} because every leaf costs a
 *                   synchronous {@code BlockBreakEvent} through every listening plugin, so
 *                   the canopy, not the trunk, is what sets a fell's real cost.
 ...
 */
public record FellSettings(int maxBlocks, int maxLeaves, int maxRadius, int maxHeight,
                           boolean dropLeaves) {
}
